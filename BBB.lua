--[[
 ...
 | __ )| __ ) / ___|   |  \/  | __ _  ___ _ __ ___| |   (_) |__ 
 |  _ \|  _ \| |  _    | |\/| |/ _` |/ __| '__/ _ \ |   | | '_ \
 | |_) | |_) | |_| |   | |  | | (_| | (__| | | (_) | |___| | |_) |
 |____/|____/ \____|   |_|  |_|\__,_|\___|_|  \___/|_____|_|_.__/ 

 BBG Macro UI Library v2
 CreateMacroWindow({title, slots, onStart, onStop})
 Returns: { ScreenGui, Frame, Logo, SlotData, SetRunning, Destroy }
]]
...
local function CreateMacroWindow(opts)
    ...
    -- (dropMenu forward declared for drag close)
    local dropMenu
    local activeDropBtn = nil

    logo.MouseButton1Click:Connect(function()
        frame.Visible = not frame.Visible
        if not frame.Visible and dropMenu then
    ...
    -- Shared Dropdown
    dropMenu = Instance.new("Frame", sg)
    dropMenu.Active                 = false
    ...
    UserInputService.InputBegan:Connect(function(input)
        ...
        or input.UserInputType == Enum.UserInputType.Touch then
            ...
            local inside  = mp.X >= absPos.X and mp.X <= absPos.X + absSize.X
                and mp.Y >= absPos.Y and mp.Y <= absPos.Y + absSize.Y
    ...
    for i = 1, slotCount do
        ...
        local dLbl = Instance.new("TextLabel", row1)
        dLbl.Size = UDim2.new(0, 36, 1, 0); dLbl.Position = UDim2.new(0, 0, 0, 0)
        dLbl.BackgroundTransparency = 1; dLbl.Text = "Delay"
        dLbl.Font = T.font; dLbl.TextSize = 10; dLbl.TextColor3 = T.subtext
        dLbl.TextXAlignment = Enum.TextXAlignment.Left

        makeInput(row1, 38, 50, "0.05", function(n) slot.delay = n end)

        local hLbl = Instance.new("TextLabel", row1)
        hLbl.Size = UDim2.new(0, 30, 1, 0); hLbl.Position = UDim2.new(0, 96, 0, 0)
        hLbl.BackgroundTransparency = 1; hLbl.Text = "Hold"
        hLbl.Font = T.font; hLbl.TextSize = 10; hLbl.TextColor3 = T.subtext
        ...
        tLbl.Size = UDim2.new(0, 36, 1, 0); tLbl.Position = UDim2.new(0, 0, 0, 0)
        tLbl.BackgroundTransparency = 1; tLbl.Text = "Type"
        tLbl.Font = T.font; tLbl.TextSize = 10; tLbl.TextColor3 = T.subtext
        ...
        -- Row 3: Move + Enable toggle
        local row3 = Instance.new("Frame", container)
        row3.Size                   = UDim2.new(1, -12, 0, 24)
        row3.Position               = UDim2.new(0, 6, 0, 82)
        row3.BackgroundTransparency = 1

        local mLbl = Instance.new("TextLabel", row3)
        mLbl.Size = UDim2.new(0, 36, 1, 0); mLbl.Position = UDim2.new(0, 0, 0, 0)
        mLbl.BackgroundTransparency = 1; mLbl.Text = "Move"
        mLbl.Font = T.font; mLbl.TextSize = 10; mLbl.TextColor3 = T.subtext
